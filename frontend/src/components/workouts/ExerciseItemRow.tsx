...
interface ExerciseItemRowProps {
  ...
  onChange: (index: number, field: keyof ExerciseItem, value: string | SetItem[]) => void;
...
export default function ExerciseItemRow({
  ...
  const hasSetDetails = exercise.setDetails && exercise.setDetails.length > 0;

  const handleToggleDetailedMode = () => {
    if (hasSetDetails) {
      // Switch to simple mode - clear setDetails
      onChange(index, 'setDetails', []);
    } else {
      // Switch to detailed mode - create initial sets based on current sets value
      const numSets = exercise.sets ?? 1;
      const newSetDetails: SetItem[] = [];
      ...
      onChange(index, 'setDetails', newSetDetails);
    }
  };

  const handleSetChange = (setIndex: number, field: keyof SetItem, value: string | boolean) => {
  ...
  const handleAddSet = () => {
    const newSetDetails = exercise.setDetails ?? [];
    const lastSet = newSetDetails[newSetDetails.length - 1];
    newSetDetails.push({
      id: crypto.randomUUID(),
      setNumber: newSetDetails.length + 1,
    ...
    <div className="flex flex-col gap-2 p-3 bg-gray-800 rounded-lg">
      ...
      {isCardio ? (
        ...
        <>
          ...
          {hasSetDetails && (
            <div className="space-y-1 border-l-2 border-gray-700 mt-1">
              {exercise.setDetails!.map((setItem, setIndex) => (
                <SetRow
                  key={setItem.id}
                  setItem={setItem}
                  onChange={(field, value) => handleSetChange(setIndex, field, value)}
                  onRemove={() => handleRemoveSet(setIndex)}
                  canRemove={exercise.setDetails!.length > 1}
                />
              ))}
          ...
          >
            {hasSetDetails ? 'Use simple mode' : 'Track each set individually'}
          </button>
        </>
      )}
    </div>
